...
This section summarizes accuracy/loss curves and key validation metrics for both **Local (RTX 4060 Ti)** and **AWS (g5.xlarge, A10G)** runs. Plots are saved in `reports/<RUN>/`.

### 7.1 Plots

**Local (r50_imagenet1k_onecycle_amp_bs64_ep150)**
- Accuracy: ![Accuracy — Local](reports/r50_imagenet1k_onecycle_amp_bs64_ep150/accuracy_curve.png)
- Loss:     ![Loss — Local](reports/r50_imagenet1k_onecycle_amp_bs64_ep150/loss_curve.png)

**AWS (imagenet1kfull_g5x_1gpu_dali_nvme_lr0p125_bs256_e150_work6)**
- Accuracy: ![Accuracy — AWS](reports/imagenet1kfull_g5x_1gpu_dali_nvme_lr0p125_bs256_e150_work6/accuracy_curve.png)
- Loss:     ![Loss — AWS](reports/imagenet1kfull_g5x_1gpu_dali_nvme_lr0p125_bs256_e150_work6/loss_curve.png)

> Plots are generated by the training script and saved per run under `reports/<RUN>/`.

---

### 7.2 Key validation metrics (auto-filled)

The table below is generated from `out/<RUN>/train_log.csv` by `update_readme.py`.  
...
### 7.3 Notes
“Throughput” comes from the per-epoch CSV column (falls back to ips or img/s if names differ).

“Epoch @ best Top-1” is the validation epoch index with the highest Top-1.

---

### 7.4 Summary of Findings

Both local and AWS trainings reached strong convergence on ImageNet-1k using identical OneCycleLR and AMP setups, but differed in speed and resource efficiency:

| Aspect | Local (RTX 4060 Ti 16 GB) | AWS g5.xlarge (A10G 24 GB) |
|---------|---------------------------|-----------------------------|
| Epoch time | ~60 min / epoch | ~30 min / epoch |
| Total time | ~240 hrs (250 epochs) | ~90 hrs (195 epochs) |
| Best Val Top-1 | 77.82% | 77.66% |
| Best Val Top-5 | 93.82% | 93.84% |
| Batch Size | 64 | 256 |
| Training Cost | ≈ $0 (local) | ≈ $85 (spot + EBS) |
